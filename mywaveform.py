...
def generate_waveformAuto(waveform_type,min_frequency, max_frequency, duration, volume, frequency_type, loop):
...
def generate_waveformManual(waveform_type,min_frequency, max_frequency, duration, volume, frequency_type, loop, step):
    sample_rate = 44100  # Adjust as needed
    buffer_size = int(sample_rate * duration)
    t = np.linspace(0, duration, buffer_size, endpoint=False)
    report_frequency = np.zeros(buffer_size)
    ...
    for i in range(buffer_size):
        if frequency_type == "Ascending":
            current_frequency = min(max_frequency, current_frequency+step)
        elif frequency_type == "Descending":
            current_frequency = max(min_frequency, current_frequency-step)
        elif frequency_type == "Keeping":
            current_frequency = min_frequency
        elif frequency_type == "Random":
            current_frequency = min_frequency + np.random.random() * (max_frequency - min_frequency)
...
def generate_waveformDouble(waveform_type,min_frequency, max_frequency, duration, volume, frequency_type, loop):
    sample_rate = 44100  # Adjust as needed
    buffer_size = int(sample_rate * duration)
    t = np.linspace(0, duration, buffer_size, endpoint=False)
    report_frequency = np.zeros(buffer_size)
    ...
    for i in range(buffer_size):
        if frequency_type == "Ascending":
            current_frequency = min(max_frequency, current_frequency*2)
        elif frequency_type == "Descending":
            current_frequency = max(min_frequency, current_frequency/2)
        elif frequency_type == "Keeping":
            current_frequency = min_frequency
        elif frequency_type == "Random":
            current_frequency = min_frequency + np.random.random() * (max_frequency - min_frequency)
